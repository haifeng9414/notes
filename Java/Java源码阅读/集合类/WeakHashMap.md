...
public class WeakHashMap<K,V>
     ...
     */
    // 和JDK7的indexFor一样，容量是2的整数倍所以可以这么求index
     ...
     */
    // 从数组中删除queue中的结点，在getTable、size、get和getEntry（间接调用getTable方法）、put等方法中都有调用
    // Entry的构造函数传入了引用队列queue，每当entry的key被回收时entry都会被添加到queue中（因为entry是WeakReference类型的，对于弱引用，
    ...
    void resize(int newCapacity) {
        ...
        // 相当于rehash，oldTable的数据都添加到newTable
        ...
        } else { // 如果resize后的大小小于阈值的一半则回滚上面的传输过程，因为getTable会执行expungeStaleEntries，可能会删除很多的key为null的entry，
            // 此时容量可能变得很小，就没必要再扩容了
            expungeStaleEntries();
